...
const REQUIRED_CHECKS: RequiredCheck[] = [
  {
    ...
    requiredSnippets: [
      "E5.2 gate integration coverage: 9 files / 20 tests expected",
      "E5.3 gate coverage: Stage 5.2 gate (9 files / 20 tests) + observability regression (1 file / 3 tests)",
  ...
  },
  {
    ...
    requiredSnippets: ["integration_tests_passed=20/20"]
  },
  {
    file: "docs/etapa-5/guia-gate-cierre-e5-3.md",
    requiredSnippets: ["stage5_gate_tests=20/20", "obs_regression_tests=3/3"]
  },
  {
...
function validateCiFileHasNoLegacyMetrics(): string[] {
  ...
  const legacyPatterns = [/23\/23/g, /23 tests/g];
