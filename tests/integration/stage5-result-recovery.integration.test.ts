...
describe("Etapa 5.2 T01 resultado durable", () => {
  it("recupera resultado por executionId tras reinicio", async () => {
    const org = await request(app).post("/api/v1/organizations").send({ name: "Org T01" });
    const project = await request(app).post("/api/v1/projects").send({ organizationId: org.body.data.id, name: "Project T01" });
    ...
    // Simula reinicio de proceso: vaciamos estado en memoria y consultamos resultado durable.
    resetStore();
    ...
    expect(recovered.body.data.entryUrl).toBe(`${labBaseUrl}/sitio-a`);
    expect(recovered.body.data.statusHttp).toBe(200);
    expect(recovered.body.data.evidenceId).toBe(crawl.body.data.evidenceId);
    expect(recovered.body.data.fetchedAt).toBeTypeOf("string");
  });
});
