...
describe("Etapa 5.2 T03 consulta operativa", () => {
  ...
  it("aplica ventana temporal y limite de resultados", async () => {
    const org = await request(app).post("/api/v1/organizations").send({ name: "Org T03-Window" });
    const project = await request(app).post("/api/v1/projects").send({ organizationId: org.body.data.id, name: "Project T03-Window" });
    ...
    // Marca temporal para separar la siguiente ejecucion en la ventana.
    ...
    const windowedItems = windowed.body.data.items as Array<{ executionId: string }>;

    expect(windowedItems.some((item) => item.executionId === failedExecution.body.data.id)).toBe(true);
    expect(windowedItems.some((item) => item.executionId === completedExecution.body.data.id)).toBe(false);
